...
bool GMM::LoadFile( char *filename )
{
...
float GMM::GetProbability( CvMat * Sample )
{
    ...
    for ( int k = 0 ; k < _nMixture ; k ++ )
    {
        ...
        expo *= (-0.5);
        P += ( _Weight[ k ] *
               1. / ( pow( 2* CV_PI, 1.5 ) * sqrt( cvDet( _CovMat[k] ) ) ) *
               exp( expo )
             );
    }
...
float GMM::GetProbabilityByLookup(int R, int G, int B)
{
    return _Probability[R][G][B];
}

bool GMM::SaveLookUpTable( char * filename )
{
...
bool GMM::LoadLookUpTable( char * filename )
{
